<template>
    <div>
        ...
        <div class="drag">
            <h2>不拷贝拖拽</h2>
            <draggable id="list1" :list="list1" class="dragArea" :options="{group:{name:'people', put:false }}">
                <div v-for="(element, index) in list1"  :key="index">
                    {{element.name}} {{index}}
                </div>
             </draggable>
             <h2>拷贝拖拽</h2>
             <draggable id="list3" :list="list3" class="dragArea" :options="{group:{name:'people', pull:'clone', put:false }}">
